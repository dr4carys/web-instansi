const getCaptcha = async () => {
  const req = await fetch(
    "https://api.sipandu-beradat.id/captcha/"
  );
  const {
    status_code,
    data
...
const getMe = async () => {
  ...
  const req = await fetch("https://api.sipandu-beradat.id/admin-desa-adat/me/", {
    method: "POST",
    body: fd
  })
  const {
...
$("form").submit(async (e) => {
  ...
  const req = await fetch(
    "https://api.sipandu-beradat.id/admin-desa-adat/login/", {
      method: "POST",
      body: fd,
    }
  );
